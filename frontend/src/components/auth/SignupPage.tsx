...
export const SignupPage: React.FC<SignupPageProps> = ({
  ...
  const getPasswordStrength = (pwd: string) => {
    if (!pwd) return { score: 0, label: '', color: 'bg-slate-700', text: 'text-slate-500' };
    let score = 0;
    ...
    if (score === 3) return { score: 3, label: 'Good', color: 'bg-indigo-500', text: 'text-indigo-400' };
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 flex flex-col justify-between relative overflow-hidden font-sans selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-8 z-10">
        <div className="w-full max-w-md">
          <div className="relative rounded-3xl bg-slate-900/90 border border-white/10 p-7 sm:p-9 shadow-2xl backdrop-blur-2xl">
            {/* Top Glowing Edge */}
            <div className="absolute top-0 inset-x-12 h-px bg-gradient-to-r from-transparent via-violet-500/50 to-transparent"></div>
            ...
            <div className="text-center mb-6 space-y-1.5">
              ...
              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed max-w-xs mx-auto">
                Turn your career and placement emails into personalized daily preparation.
              </p>
            </div>
            ...
            {isSuccess && (
              <div className="mb-5 p-3 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-xs flex items-center gap-2.5 animate-in fade-in duration-200">
                ...
                <span className="font-medium">Account created! Redirecting to setup...</span>
              </div>
            )}
                ...
                {password && (
                  <div className="pt-1 space-y-1 animate-in fade-in duration-200">
                    ...
                    <div className="grid grid-cols-4 gap-1.5 h-1.5 w-full">
                      ...
                      <div
                        className={`rounded-full transition-colors ${
                          strength.score >= 4 ? strength.color : 'bg-slate-800'
                        }`}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="pt-1">
                <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-300 select-none">
                  <input
                    ...
                    className="w-4 h-4 mt-0.5 rounded bg-slate-950 border border-white/20 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-slate-900 shrink-0"
                  />
      ...
      <footer className="w-full py-5 text-center text-xs text-slate-500 border-t border-white/5 z-10">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <span>&copy; {new Date().getFullYear()} PrepPilot. Free for college students.</span>
          <div className="flex items-center gap-4 text-slate-400">
            <span className="flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span>Zero Spam • Privacy First</span>
            </span>
          </div>
        </div>
      </footer>
